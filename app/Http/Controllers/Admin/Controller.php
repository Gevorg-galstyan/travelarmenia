            ...
            case 'multiple_images':
                if ($files = $request->file($row->field)) {
                    ...
                    foreach ($files as $key => $file) {
                        ...
                        if (!empty($request->multiple_watermark)) {
                            $bottom = $request->bottom ?? 20;
                            $right = $request->right ?? 20;
                            $watermark = Image::make(storage_path('app/public/' . $request->multiple_watermark))->opacity(50);
                            $image->insert($watermark, 'bottom-right', $bottom, $right);
                        }
                        $resize_width = null;
                        $resize_height = null;
                            ...
                            foreach ($options->thumbnails as $thumbnails) {
                                if (isset($thumbnails->name) && isset($thumbnails->scale)) {
                                    ...
                                    if (!empty($request->multiple_watermark)) {
                                        ...
                                        $watermark = Image::make(storage_path('app/public/' . $request->multiple_watermark))->opacity(50);
                                        $image = Image::make($file)
                                            ->insert($watermark, 'bottom-right', $bottom, $right)
                                            ->resize(
                                                $thumb_resize_width,
                                                $thumb_resize_height,
                                                function (Constraint $constraint) use ($options) {
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    if (!empty($request->watermark)) {
                        $bottom = $request->bottom ?? 20;
                        $right = $request->right ?? 20;
                        $watermark = Image::make(storage_path('app/public/' . $request->watermark))->opacity(50);
                        $image->insert($watermark, 'bottom-right', $bottom, $right);
                    }
