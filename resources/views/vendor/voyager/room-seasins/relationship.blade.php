        ...
        @if($options->type == 'belongsTo')
            ...
            @else
                <select class="form-control select2" name="{{ $options->column }}">
                    @php
                        $model = app($options->model);
                        if($options->model == 'App\Models\Room'){
                            $query = $model::where('id', session('parent_id'))->get();
                        }elseif ($options->model == 'App\Models\Season'){
                            $query = $model::where('hotel_id', $hotel_id)->get();
                        }else{
                            $query = $model::all();
                        }
                    @endphp
                    ...
                    @foreach($query as $relationshipData)
                        ...
                        <option value="{{ $relationshipData->{$options->key} }}" @if($dataTypeContent->{$options->column} == $relationshipData->{$options->key}){{ 'selected="selected"' }}@endif
                                {{session('parent_id') && session('parent_id') == $relationshipData->{$options->key}  ? 'selected':''}}>{{ $relationshipData->{$options->label} }}</option>
                    @endforeach
                </select>
