...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <h5>Локатион фильтр </h5>
                    <div class="btn-group btn-group-sm" role="group">
                        <label class="btn btn-primary {{$loc == 'ru' ? 'active' :''}}">
                            <a href="?lang=ru" style="color: #ffffff;">RU</a>
                        </label>
                        <label class="btn btn-primary {{$loc == 'en' ? 'active' :''}}">
                            <a href="?lang=en" style="color: #ffffff;">EN</a>
                        </label>
                    </div>
                    <button type="button" data-toggle="modal"  data-target="#send-subscribers"
                            class="btn btn-success  open-subscriber-modal" title="выберите минимум 1 эмиль" disabled>
                        <i class="fas fa-share-square"></i> <span>отправить </span>
                    </button>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <td>
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    <div class="readmore" {{$row->field == 'email'? "data-status=".$data->getKey() : ''}}>{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</div>
    ...
    {{--Subscriber Modal--}}
    <div id="send-subscribers" class="modal modal-success fade" role="dialog">
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Send Subscribers</h4>
                </div>
                <div class="modal-body">
                    <form action="{{route('send_subscribers')}}" method="post" class="subscriber-form">
                        {{csrf_field()}}
