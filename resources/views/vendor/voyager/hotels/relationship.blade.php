        ...
        @elseif($options->type == 'hasMany')
            ...
            @else
                ...
                @if(isset($query))
                    ...
                    @if($options->model == 'App\Models\Season')
                        <a href="/admin/seasons/create?parent={{$dataTypeContent->id}}" class="btn btn-success" title="Add Season"><i
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                @foreach($query as $i => $query_res)
                                    <tr>
                                        <td>{{$i + 1}}</td>
                                        <td>{{ $query_res->start }}</td>
                                        <td>{{$query_res->end}}</td>
                                        <td>
                                            <a href="/admin/seasons/{{$query_res->id}}/edit?parent={{$query_res->hotel_id}}"
                                               class="btn btn-info">
                                                Edit
                                            </a>
                                        </td>
                    ...
                    @elseif($options->model == 'App\Models\Room')
                        <ul>
                            <div class="table-responsive">
                                <table class="table">
                                    <thead>
                                    ...
                                    @foreach($query as $i => $query_res)
                                        <tr>
                                            <td>{{$i + 1}}</td>
                                            <td><span>{{ $query_res->{$options->label} }}</span></td>
                                            <td>
                                                <a href="/admin/rooms/{{$query_res->id}}/edit?parent=1"
                                                   class="btn btn-info">
                                                    Edit
                                                </a>
                                            </td>
